...
import prefuse.data.util.NeighborList;
 ...
 */
public class Graph <T extends Tuple<?>, N extends Node<N,E>, E extends Edge<N,E>> extends CompositeTupleSet<T> {
    ...
    /** Listener list */
    private final CopyOnWriteArrayList<GraphListener> m_listeners = new CopyOnWriteArrayList<GraphListener>();
     ...
     */
    public static Graph<TableTuple<?>,TableNode,TableEdge> createGraph() {
     ...
     */
    public static Graph<TableTuple<?>,TableNode,TableEdge> createGraph(boolean directed) {
    	return createGraph(new Table<TableNode>(0,0) {
			@Override
     ...
     * @param directed true for directed edges, false for undirected
     */
    public static Graph<TableTuple<?>,TableNode,TableEdge> createGraph(Table<TableNode> nodes, boolean directed) {
    	return createGraph(nodes, directed, DEFAULT_NODE_KEY,
     ...
     * table
     */
    public static Graph<TableTuple<?>,TableNode,TableEdge> createGraph(Table<TableNode> nodes, boolean directed, String nodeKey,
            String sourceKey, String targetKey) {
        Table<TableEdge> edges = new Table<TableEdge>() {
			@Override
        ...
        return new Graph<TableTuple<?>,TableNode,TableEdge>(nodes,edges,directed,nodeKey,sourceKey, targetKey);
    }

    public static Graph<TableTuple<?>,TableNode, TableEdge> createGraph(Table<TableNode> nodes, Table<TableEdge> edges, boolean directed) {
    	return new Graph<TableTuple<?>,TableNode, TableEdge>(nodes, edges, directed);
    }
     ...
     * @param directed true for directed edges, false for undirected
     */
    public Graph(Table<N> nodes, Table<E> edges, boolean directed) {
     ...
     * table
     */
    protected void init(final Table<N> nodes, final Table<E> edges, boolean directed,
            String nodeKey, String sourceKey, String targetKey)
    {
        // sanity check
        if ( nodeKey!=null &&
        ...
        {
        	if(edges.getColumnType(sourceKey) == null) {
        		throw new IllegalArgumentException("null edge source");
        	}
        	if(edges.getColumnType(targetKey) == null) {
        		throw new IllegalArgumentException("null edge target");
        	}
        ...
        removeAllSets();
        super.addSet(EDGES, (TupleSet<? extends T>) edges);
        super.addSet(NODES, (TupleSet<? extends T>) nodes);
        ...
        if ( m_nodeTuples == null ) {
            m_nodeTuples = new TupleManager<N>(nodes, this) {
				public N createTupleInstance() {
					return nodes.createTupleInstance();
				}
            };
        }
        m_edgeTuples = new TupleManager<E>(edges, this) {
			public E createTupleInstance() {
				return edges.createTupleInstance();
			}
        };
    ...
    protected Table<TableTuple<?>> createLinkTable() {
    	Table<TableTuple<?>> linkTable = Table.createTable(getNodeTable().getMaximumRow()+1, LINKS_SCHEMA.getColumnCount());
    	linkTable.addColumns(LINKS_SCHEMA);
        return linkTable;
    }
    ...
    public N addNode() {
        int nrow = addNodeRow();
        return m_nodeTuples.getTuple(nrow);
    }
    ...
    public boolean removeNode(int node) {
        Table<N> nodeTable = getNodeTable();
...
//      if ( ng != this && ng.m_spanning != this ) {
     ...
     */
    public TupleSet<N> getNodes() {
        return (Table<N>) getSet(NODES);
    }
    ...
    public N getNode(int n) {
        return m_nodeTuples.getTuple(n);
    }
     ...
     */
    public N getNodeFromKey(long key) {
     ...
     */
    public TupleSet<E> getEdges() {
        return (TupleSet<E>) getSet(EDGES);
    }
     ...
     */
    public Table<E> getEdgeTable() {
        return (Table<E>) getSet(EDGES);
    }
    /**
    ...
    public E getEdge(int e) {
        return e < 0 ? null : m_edgeTuples.getTuple(e);
    }
     ...
     * such edge is found.
     */
    public E getEdge(Node<?,?> source, Node<?,?> target) {
     ...
     */
    public N getAdjacentNode(Edge<?,?> e, Node<?,?> n) {
        edgeCheck(e, true);
        nodeCheck(n, true);
     ...
     * node
     */
    public List<Integer> edgeRows(int node) {
    ...
    public List<Integer> edgeRows(int node, int direction) {
        if ( direction==OUTEDGES ) {
            ...
            return new IntArrayList(outedges, getOutDegree(node));
        } else if ( direction==INEDGES ) {
            int[] inedges = (int[])m_links.get(node, INLINKS);
            return new IntArrayList(inedges, getInDegree(node));
    ...
    protected static class IntArrayList extends AbstractList<Integer> implements RandomAccess {

    	private final int[] ints;
    	private final int len;

    	public IntArrayList(int[] ints, int len) {
    		this.ints = ints;
    		this.len = len;
    	}
     ...
     */
    public List<Integer> inEdgeRows(int node) {
     ...
     */
    public List<Integer> outEdgeRows(int node) {
    ...
    public Iterable<N> nodes() {
        return m_nodeTuples.iterable(nodeRows());
    }
    ...
    public List<N> neighbors(N n) {
        return new NeighborList<N,E>(n, edges(n));
    }
     ...
     */
    public List<N> inNeighbors(N n) {
        return new NeighborList<N,E>(n, inEdges(n));
    }
     ...
     */
    public List<N> outNeighbors(N n) {
        return new NeighborList<N,E>(n, outEdges(n));
    }
    ...
    public Iterable<E> edges() {
        return m_edgeTuples.iterable(edgeRows());
    }
     ...
     */
    public List<E> edges(N node) {
        nodeCheck(node, true);
        return m_edgeTuples.list(edgeRows(node.getRow(), UNDIRECTED));
    }
     ...
     */
    public List<E> inEdges(N node) {
        nodeCheck(node, true);
        return m_edgeTuples.list(inEdgeRows(node.getRow()));
    }
     ...
     * node
     */
    public List<E> outEdges(N node) {
        nodeCheck(node, true);
        return m_edgeTuples.list(outEdgeRows(node.getRow()));
    }
	...
	public Iterable<T> tuples(Predicate filter) {
        if ( filter == null ) {
            ...
            return new CompositeIterable<T>(
            		(Iterable<T>)m_edgeTuples.iterable(getEdgeTable().rows(filter)),
            		(Iterable<T>)m_nodeTuples.iterable(getNodeTable().rows(filter)));
        }
    }
	...
	public Iterable<T> tuples() {
        return new CompositeIterable<T>((Iterable<T>)m_edgeTuples.iterable(edgeRows()), (Iterable<T>)m_nodeTuples.iterable(nodeRows()));
    }
    ...
    public DeclarativeTree<N,E> getSpanningTree() {
        if ( m_spanning == null ) {
			return getSpanningTree(nodes().iterator().next());
		} else {
			return m_spanning;
		}
    }
            ...
            int first, int last, int col, int type)
    {
		...
		for (GraphListener l : m_listeners) {
			l.graphChanged(this, table, first, last, col, type);
		}
    }

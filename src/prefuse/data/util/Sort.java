...
public class Sort {
    ...
    public <T extends Tuple<?>> Comparator<T> getComparator(TupleSet<T> ts) {
        ...
        if ( ts instanceof Table ) {
            ...
            s = ts.tuples().iterator().next().getSchema();
        }
        // create the comparator
        CompositeComparator<T> cc = new CompositeComparator<T>(m_fields.length);
        for ( int i=0; i<m_fields.length; ++i ) {
            cc.add(new TupleComparator<T>(m_fields[i],

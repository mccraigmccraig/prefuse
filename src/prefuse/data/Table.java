 ...
 */
public abstract class Table <T extends Tuple<?>> extends AbstractTupleSet<T> implements ColumnListener {

    public static Table<TableTuple<?>> createTable() {
    	return createTable(0,0);
	}

    public static Table<TableTuple<?>> createTable(int nrows, int ncols) {
		return new Table<TableTuple<?>>(nrows, ncols) {
			@Override
			public TableTuple<?> createTupleInstance() {
				return new TableTuple<TableTuple<?>>();
			}};
	}

    public static Table<TableTuple<?>> createTable(Schema schema) {
    	Table<TableTuple<?>> table = createTable();
     ...
     */
    protected Table(int nrows, int ncols) {
        ...
        final Table<T> localThis = this;
        m_tuples = new TupleManager<T>(this, null) {

			@Override
			public T createTupleInstance() {
				return localThis.createTupleInstance();
			}
     ...
     */
    public void setTupleManager(TupleManager<T> tm) {
    ...
    protected void updateRowCount() {
        ...
        // update columns
        Iterable<Column> cols = getColumns();
        for ( Column c : cols ) {
     ...
     */
    protected Iterable<Column> getColumns() {
     ...
     */
    protected Iterable<String> getColumnNames() {
     ...
     * Tuple instance.
     * @param t the Tuple to make the sole tuple in this table
     * @return the actual Tuple instance added to this table, or null if
     * no new Tuple has been added
     ...
     */
    public Table<T> createEmptyCopyWithSchema(Schema schema) {
    	int ncols = schema.getColumnCount();

    	final Table<T> thisLocal = this;

    	Table<T> t = new Table<T>(0,ncols) {

			@Override
			public T createTupleInstance() {
				return thisLocal.createTupleInstance();
			}
     ...
     */
    public Table<T> select(Predicate filter, Sort sort) {

    	Table<T> t = createEmptyCopyWithSchema(getSchema());

        Iterable<T> tuples = tuples(filter, sort);
        for ( T tuple : tuples) {
            t.addTuple(tuple);
        }
        return t;
     ...
     */
    public Iterable<T> tuplesReversed() {
        return m_tuples.iterable(rows(true));
    }
     ...
     */
    public Iterable<T> tuples(IntIterator rows) {
        return m_tuples.iterable(rows);
    }
    ...
    public IntIterator rows(Predicate filter) {
        return FilterIterableFactory.rows(this, filter);
    }
    ...
    protected void fireTableEvent(int row0, int row1, int col, int type) {
		...
		for (TableListener l : m_listeners) {
			l.tableChanged(this, row0, row1, col, type);
        }
    }

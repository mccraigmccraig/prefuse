 ...
 */
public class CompositeTupleSet <T extends Tuple<?>> extends AbstractTupleSet<T> {

    private static final Logger s_logger
        = Logger.getLogger(CompositeTupleSet.class.getName());

    private Map<String, TupleSet<? extends T>> m_map;   // map names to tuple sets
    private Set<TupleSet<? extends T>> m_sets;  // support quick reverse lookup
    ...
    protected CompositeTupleSet(boolean listen) {
        m_map = new LinkedHashMap<String, TupleSet<? extends T>>();
        m_sets = new HashSet<TupleSet<? extends T>>();
        m_count = 0;
     ...
     */
    public void addSet(String name, TupleSet<? extends T> set) {
        if ( hasSet(name) ) {
     ...
     * false otherwise
     */
    public boolean containsSet(TupleSet<? extends Tuple<?>> set) {
     ...
     */
    public TupleSet<? extends T> getSet(String name) {
     ...
     */
    public Iterable<String> setNames() {
     ...
     */
    public Iterable<TupleSet<? extends T>> sets() {
     ...
     */
    public TupleSet<? extends T> removeSet(String name) {
        TupleSet<? extends T> ts = m_map.remove(name);
        if ( ts != null ) {
            m_sets.remove(ts);
            if ( m_lstnr != null ) {
    ...
    public void removeAllSets() {
        Iterator<Map.Entry<String, TupleSet<? extends T>>> sets = m_map.entrySet().iterator();
        while ( sets.hasNext() ) {
        	Map.Entry<String, TupleSet<? extends T>> entry = sets.next();
            TupleSet<? extends T> ts = entry.getValue();
            sets.remove();
            m_sets.remove(ts);
            if ( m_lstnr != null ) {
				ts.removeTupleSetListener(m_lstnr);
    ...
    public boolean containsTuple(Tuple<?> t) {
        Iterator<Map.Entry<String, TupleSet<? extends T>>> it = m_map.entrySet().iterator();
        while ( it.hasNext() )  {
        	Map.Entry<String, TupleSet<? extends T>> entry = it.next();
            TupleSet<? extends T> ts = entry.getValue();
    ...
    public Iterable<T> tuples() {
        CompositeIterable<T> ci = new CompositeIterable<T>(m_map.size());
        Iterator<Map.Entry<String, TupleSet<? extends T>>> it = m_map.entrySet().iterator();
        for ( int i=0; it.hasNext(); ++i )  {
        	Map.Entry<String, TupleSet<? extends T>> entry = it.next();
            TupleSet<? extends T> ts = entry.getValue();

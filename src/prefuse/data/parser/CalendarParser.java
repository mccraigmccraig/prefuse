...
import prefuse.data.parser.CalendarParserException.ExceptionTypes;
...
class ParserState {
	 ...
	 *            calendar to fill
	 * @param ignoreChanges
	 *            if <tt>true</tt>, throw an exception when a date like <tt>Sept 31</tt> is
	 *            changed to <tt>Oct 1</tt>
	 * 
	 * @throws CalendarParserException
	 *             if the date cannot be set for some reason
	 */
	void setCalendar(final GregorianCalendar cal, boolean ignoreChanges) throws CalendarParserException {
		cal.clear();
	 ...
	 */
	void setDate(final int val) throws CalendarParserException {
		if (val < 1 || val > 31) {
	 ...
	 */
	void setHour(final int val) throws CalendarParserException {
		final int tmpHour;
		if (timePostMeridian) {
			tmpHour = val + 12;
			timePostMeridian = false;
	 ...
	 */
	void setMillisecond(final int val) throws CalendarParserException {
		if (val < 0 || val > 999) {
	 ...
	 */
	void setMinute(final int val) throws CalendarParserException {
		if (val < 0 || val > 59) {
	 ...
	 */
	void setMonth(final int val) throws CalendarParserException {
		if (val < 1 || val > 12) {
	 ...
	 */
	void setSecond(final int val) throws CalendarParserException {
		if (val < 0 || val > 59) {
	 ...
	 */
	void setTimePostMeridian(final boolean val) {
	 ...
	 *            time zone
	 */
	void setTimeZone(final TimeZone tz) {
		timeZone = tz;
	}
	 ...
	 */
	void setYear(final int val) throws CalendarParserException {
		if (val < 0) {
...
public class CalendarParser {
	 ...
	 */
	private static final void appendTimeString(final StringBuffer buf, final Calendar cal, final boolean needSpace) {
	 ...
	 * @return order string
	 */
	public static final String getOrderString(final int order) {
		switch (order) {
			case DD_MM_YY:
				return "DD_MM_YY";
			case MM_DD_YY:
	 ...
	 * For example, <tt>"1st"</tt> would return <tt>1</tt>, <tt>"23rd"</tt> would return
	 * <tt>23</tt>, etc.
	 * 
	 * @param str
	 *            ordinal string
	 * 
	 * @return the numeric value of the ordinal number, or <tt>CalendarParser.UNSET</tt> if the
	 *         supplied string is not a valid ordinal number.
	 */
	private static final int getOrdinalNumber(final String str) {
		final int len = str == null ? 0 : str.length();
		if (len >= 3) {

			final String suffix = str.substring(len - 2);
	 ...
	 */
	private static final String getTimePlaceString(final int place) {
		switch (place) {
			case PLACE_HOUR:
				return "hour";
			case PLACE_MINUTE:
	 ...
	 */
	private static final boolean isWeekdayName(final String str) {
		if (str == null || str.length() < 3) {
			return false;
		}

		final String lstr = str.toLowerCase();
		for (final String element : WEEKDAY_NAMES) {
	...
	private static final String[] loadTimeZoneNames() {
		...
		final String[] tmpList = (String[]) result;
	 ...
	 * For example, <tt>"January"</tt> (or any substring) would return <tt>1</tt> and
	 * <tt>"December"</tt> would return <tt>12</tt>.
	 * 
	 * @param str
	 *            month name
	 * 
	 * @return the numeric month, or <tt>CalendarParser.UNSET</tt> if the supplied string is not a
	 *         valid month name.
	 */
	public static int monthNameToNumber(final String str) {
		if (str != null && str.length() >= 3) {
			final String lstr = str.toLowerCase();
	 ...
	 */
	public static final Calendar parse(final String dateStr) throws CalendarParserException {
	 ...
	 *            date string
	 * @param order
	 *            order in which pieces of numeric strings are assigned (should be one of
	 *            <tt>YY_MM_DD</tt>, <tt>MM_DD_YY</tt>, etc.)
	 * 
	 * @return parsed date
	 * 
	 ...
	 *             if there was a problem parsing the string.
	 */
	public static final Calendar parse(final String dateStr, final int order) throws CalendarParserException {
	 ...
	 *            <tt>YY_MM_DD</tt>, <tt>MM_DD_YY</tt>, etc.)
	 * @param ignoreChanges
	 *            if <tt>true</tt>, ignore date changes such as <tt>Feb 31</tt> being changed
	 *            to <tt>Mar 3</tt>.
	 * 
	 * @return parsed date
	 * 
	 ...
	 *             if there was a problem parsing the string.
	 */
	public static final Calendar parse(final String dateStr, final int order, final boolean ignoreChanges)
			throws CalendarParserException {
		if (dateStr == null) {
			return null;
		}
	 ...
	 */
	private static final void parseNonNumericToken(final String dateStr, final ParserState state, final String token)
			throws CalendarParserException {
		...
		final int tmpMon = monthNameToNumber(token);
		...
		final int val = getOrdinalNumber(token);
		if (val == UNSET) {
			...
			if (lToken.equals("am")) {
				...
				for (final String element : zoneNames) {
					if (token.equalsIgnoreCase(element)) {
						final TimeZone tz = TimeZone.getTimeZone(token);
	 ...
	 */
	private static final void parseNumericBlob(final String dateStr, final ParserState state, final int val)
			throws CalendarParserException {
	 ...
	 *             if there was a problem parsing the token
	 */
	private static final void parseNumericToken(final String dateStr, final ParserState state, final int val)
			throws CalendarParserException {
		...
		if (val > 31) {
			...
			if (state.getYear() > 31) {
				...
				final String errMsg = "Couldn't decide on year number in date \"" + dateStr + "\"";
	 ...
	 */
	private static final Calendar parseString(final String dateStr, final int order, final boolean ignoreChanges)
			throws CalendarParserException {
		final ParserState state = new ParserState(order);

		final Pattern pat = Pattern.compile("([\\s/,]+|(\\S)\\-)");
		final Matcher matcher = pat.matcher(dateStr);
		...
		if (!state.isDateSet()) {
			if (!state.isMonthSet()) {
				if (!state.isYearSet()) {
					...
					throw new CalendarParserException("Day and month missing" + " from \"" + dateStr + "\"", ExceptionTypes.ONLY_YEAR_FOUND_EXCEPTION);
				}
			} else {
	 ...
	 */
	private static final void parseTime(final String dateStr, final ParserState state, final String timeStr)
			throws CalendarParserException {
		...
		for (final String token : tList) {
			...
			switch (place) {
				case PLACE_HOUR:
					try {
						state.setHour(val);
					} catch (final CalendarParserException dfe) {
	 ...
	 *             if there is a problem with the time
	 */
	private static final void parseTimeZoneOffset(final String dateStr, final ParserState state, final String zoneStr)
			throws CalendarParserException {
		int place = PLACE_HOUR;
		...
		final String[] tList = zoneStr.substring(1).split(":");
		for (final String token : tList) {
			if (DEBUG) {
	 ...
	 */
	public static final String prettyString(final Calendar cal) {
		if (cal == null) {
			return null;
		}
	 ...
	 */
	public static final String toString(final Calendar cal) {
		if (cal == null) {
			return null;
		}
	 ...
	 */
	public static final String toSQLString(final Calendar cal) {
		if (cal == null) {
			return null;
		}

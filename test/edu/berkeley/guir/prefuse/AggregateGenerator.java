...
import edu.berkeley.guir.prefuse.pipeline.Layout;
import edu.berkeley.guir.prefuse.pipeline.TreeEdgeFilter;
import edu.berkeley.guir.prefuse.pipeline.VerticalTreeLayout;
...
public class AggregateGenerator {
	...
	public VerticalTreeLayout layout;
	...
	public static void main(String[] args) {
		AggregateGenerator ag = new AggregateGenerator();
		ag.renderAggregates();
		System.exit(0);
	} //
	...
	public AggregateGenerator() {
		try {
			...
			tree = new TabDelimitedTreeReader().loadTree(TREE_CHI97);
			...
			while ( iter.hasNext() ) {
				...
				if ( n.getNumChildren() > 0 ) {
					...
					if ( !aggSet.contains(key) ) {					
						...
						saveImage(SAVE_DIR+key+".png",display);
						aggSet.add(key);
					}
				}
			}
	...
	public void saveImage(String filename, Display display) {
		BufferedImage image = display.getOffscreenBuffer();
		Rectangle r = getImageBounds(image);
	...
	public Rectangle getImageBounds(BufferedImage image) {
